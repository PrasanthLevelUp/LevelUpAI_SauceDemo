...
test.describe('Login Test - Both Fields Empty', () => {
  ...
  test('Test login with both username and password fields empty.', async ({ page }) => {
    ...
    // Step 2: Leave the username input empty.
    await page.getByRole('textbox', { name: /username input/i }).fill('');

    // Step 3: Leave the password input empty.
    await page.getByRole('textbox', { name: /password input/i }).fill('');
    ...
    // Step 5: Verify that the appropriate error messages are displayed.
    // Assert: Verify that the appropriate error messages are displayed.
  });
